...
/* =========================================================
   ...
   ECU → 이 보드  MTR_CMD (0x210)  주기 10ms  DLC 6  (E2E P01, SAF010 ASIL-B)
     ...
     Byte 5:              MTR_CRC (CRC-8/AUTOSAR E2E P01, poly 0x2F, init 0xFF, no XOR-out)
     NOTE:
   ...
   이 보드 → ECU,SENSOR  MTR_SPD_FB (0x310)  주기 10ms  DLC 8  (signal-level multicast)
   ...
   이 보드 → ECU  MTR_HEARTBEAT (0x320)  주기 10ms  DLC 2  (SYS025)
     Byte 0: HB_MTR (uint8, 0~255 순환)
     Byte 1: ERR_MTR (uint8, 0=정상, 비트 플래그 — ERR_* 상수 참조)

   ECU → 이 보드  ECU_HEARTBEAT (0x480)  주기 10ms  DLC 2  (SAF018, ASIL-B)
...
#define CAN_ID_MTR_SPD_FB    0x310   // 송신 ID: AVG + 4륜 속도 피드백 (DLC 8)
#define CAN_ID_MTR_HEARTBEAT 0x320   // 송신 ID: 모터 노드 HB+ERR
#define CAN_ID_ECU_HEARTBEAT 0x480   // 수신 ID: ECU HB 감시 (SAF018)
